...
export default function PopularSeries() {
  ...
  // get Popular Series frome the Seriedb, add one in topSerie ans others in popularSeries 
    const getSeries = async() => {
        const res = await fetch(`${url}/discover/tv?api_key=${api_key}&language=en-US&page=1`)
    ...
    <div>
      ...
      {topSerie && 
        <div className="relative">
          ...
          <Image src={`https://image.tmdb.org/t/p/original${topSerie.poster_path}`} alt={topSerie.title && topSerie.title || !topSerie.title ? topSerie.original_title : topSerie.original_title || 'Serie'} width={1400} height={1000} className="min-w-full min-h-screen max-h-screen object-cover object-center sm:hidden" />
